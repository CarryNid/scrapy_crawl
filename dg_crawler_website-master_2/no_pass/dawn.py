...
class DAWNSpider(BaseSpider):
    ...
    def parse_page(self, response):
        ...
        if self.time is not None:
            time = response.xpath('//span[@class="timestamp--time  timeago"]/@title').get().split()
            last_time = time[2] + '-' + month[time[1]] + '-' + time[0] + ' 00:00:00'
        if self.time is None or DateUtil.formate_time2time_stamp(last_time) >= int(self.time):
            ...
            for i in article_list:
                ...
                time = response.xpath('//span[@class="timestamp--time  timeago"]/@title').get().split()
                pub_time = time[2] + '-' + month[time[1]] + '-' + time[0] + ' 00:00:00'
